module CamaleonCms::Frontend::NavMenuHelper
  ...
  def draw_menu(args = {})
    args_def = {
                ...
                item_not_current: '', #class for current menu item
  ...
  def cama_menu_draw_items(args, nav_menu, level = 0)
    ...
    nav_menu.eager_load(:metas).each do |nav_menu_item|
      ...
      html += "<#{_args[:item_container]} #{r[:item_container_attrs]} class='#{_args[:item_class]} #{_args[:item_class_parent] if has_children} #{_is_current ? "#{_args[:item_current]}" : "#{_args[:item_not_current]}"} #{"current-menu-ancestor" if current_children }'>#{_args[:link_before]}
